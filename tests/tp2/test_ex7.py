...
class TestEx7(unittest.TestCase):
    ...
    def test_creation_compte_valide(self, mock_input, mock_pwdGenerate, mock_isValid):
        ...
        self.assertIn(
            "Login: user123, Email: user@example.com, Mot de passe: P@ssw0rd", output
        )

    @patch("ex3.isValid")
    @patch("ex5.pwdGenerate")
    @patch("builtins.input")
    def test_creation_multiple_comptes(
        self, mock_input, mock_pwdGenerate, mock_isValid
    ):
        ...
        mock_input.side_effect = [
            "2",  # nombre de comptes
            "user1",
            "user1@example.com",  # premier compte
            "autre",  # continuer
            "user2",
            "user2@example.com",  # deuxième compte
        ]
        mock_isValid.return_value = True
        ...
        self.assertIn(
            "Login: user1, Email: user1@example.com, Mot de passe: P@ssw0rd1", output
        )
        self.assertIn(
            "Login: user2, Email: user2@example.com, Mot de passe: P@ssw0rd2", output
        )
    ...
    def test_informations_invalides(self, mock_input, mock_pwdGenerate, mock_isValid):
        ...
        mock_input.side_effect = [
            "3",  # nombre de comptes
            "user@invalid",
            "user@example.com",  # première tentative invalide
            "1",  # pour continuer
            "user123",
            "user@example.com"  # deuxième tentative valide
            "-1"  # pour ne pas continuer
            "user456",
            "user2@example.com",  # deuxième tentative valide
        ]
    ...
    def test_terminer_prematurement(self, mock_input, mock_pwdGenerate, mock_isValid):
        ...
        self.assertIn(
            "Login: user1, Email: user1@example.com, Mot de passe: P@ssw0rd", output
        )
